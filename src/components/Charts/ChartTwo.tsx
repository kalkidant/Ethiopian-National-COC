...
{totalCount?.map((data:any,index:any)=>{
  return <div key={index} className="flex items-center mt-8 flex-col sm:flex-row">
    ...
    <div className="w-full sm:w-[222px] h-[10px] bg-[#98D2C1] rounded-full inline-block mb-2 sm:mb-0">
      <div className={`h-full text-center bg-[#32A583] text-xs text-white rounded-full ${data?._count?.city ? `w-[${(data?._count?.city / totalCoc)}%]` : 'w-0'}`}>
      </div>
    </div>
